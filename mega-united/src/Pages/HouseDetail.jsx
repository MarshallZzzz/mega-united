    ...
    return (
        <div className='flex flex-col bg-black'>
            ...
            items-center w-full ${blur ? "bg-black" : ""}`}
                ...
                {/* <div className="fixed"> */}
                <div className={`absolute top-0 right-0 w-1/4 h-1/2 bg-gray-800 text-white rounded-lg shadow-lg z-40 transform transition-transform duration-300
                                ${menuOpen ? "translate-x-0" : "translate-x-full"}`}>
                    <div className="absolute left-[-50px] bottom-0 -translate-y-1/2 w-16 h-16 bg-gray-800 rounded-sm">
                    ...
                    <div className='flex flex-col gap-6'>
                        ...
                        <div className='px-6'>
                            ...
                            <div className='grid grid-cols-2 justify-start pl-4 border-b border-white divide-x divide-white'>
                                <h2 className='flex flex-col text-2xl pb-6'>YEAR BUILT  <span className='text-base'>{item.yrbuilt}</span></h2>
                                <h2 className='flex flex-col text-2xl pl-4'>PRICE <span className='text-base'>{item.price}</span></h2>
                            </div>
                            <div className='grid grid-cols-2 justify-start pl-4 border-b border-white divide-x divide-white'>
                                <h2 className='flex flex-col text-2xl pb-6'>BED <span className='text-base'>{item.bed}</span></h2>
                                <h2 className='flex flex-col text-2xl pl-4'>BATH <span className='text-base'>{item.bath}</span></h2>
                            </div>
                            <div className='grid grid-cols-2 justify-start pl-4 divide-x divide-white'>
                                <h2 className='flex flex-col text-2xl pb-6'>STORIES <span className='text-base'>{item.stories}</span></h2>
                                <h2 className='flex flex-col text-2xl pl-4'>SIZE <span className='text-base'>{item.size}</span></h2>
                            </div>
                        </div>
                    </div>
                </div>
